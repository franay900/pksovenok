...
export default {
    ...
    methods: {
        ...
        getLoads(event) {
            ...
            axios.get(`/api/load/${this.selectedGroup.id}`)
                .then(res => {
...
<template>
    ...
    <div class="card flex flex-col gap-4">
        ...
        <div class="mb-4" style="width: auto;">
            ...
            <Select v-model="selectedGroup" :options="groups" optionLabel="group_name" placeholder="Выберите класс"
                    class="w-full md:w-56" @change="getLoads($event)"/>

            <DataTable class="mt-3" :value="subjects" sortMode="multiple" tableStyle="min-width: 50rem"
                       v-if="selectedGroup && subgroups">
                ...
                <Column header="Подгруппа" sortable="">
                    ...
                    <template #body="slotProps">
                        <template v-if="filteredSubgroupsForSubject[slotProps.data.id]">
                            <div class="col mt-3" v-for="subgroup in filteredSubgroupsForSubject[slotProps.data.id]"
                                 :key="subgroup.id">
                                {{ subgroup.name }}
